...
struct EarthProgram {
    ...
    EarthProgram(const FilePath& applicationPath):
        m_Program(loadProgram(applicationPath.dirPath() + "shaders/3D.vs.glsl", // shaders/pointlight.fs.glsl
                              applicationPath.dirPath() + "shaders/pointlight.fs.glsl")) { // shaders/directionallight.fs.glsl
...
int main(int argc, char** argv) {
    ...
    while(!done) {
        ...
        glUniform3f(earthProgram.uKd, 0.1f, 0.2f, 0.3f); //Couleur des boules
        glUniform3f(earthProgram.uKs, 0.5f, 0.f , 0.f );
        glUniform1f(earthProgram.uShininess, 32.f);
        glm::mat4 tryCamera  = glm::rotate(Camera.getViewMatrix(), windowManager.getTime(), glm::vec3(1, 0, 0));
        glm::vec4 LightDir = tryCamera * glm::vec4(0.0, -3.0, 0.0, 0.0);
        glUniform3f(earthProgram.uLightPos_vs, LightDir.x, LightDir.y, LightDir.z);
        glUniform3f(earthProgram.uLightIntensity, 5.f, 5.f, 5.f);
